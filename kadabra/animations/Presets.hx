...
class Presets {
	...
	public static function flash<T>(target:T, duration:Float = 1.0):T {
		...
		Actuate.tween(target, duration * .25, {alpha: 1}, false)
			.autoVisible(false)
			.ease(Quad.easeInOut)
			.delay(duration * .25);
		Actuate.tween(target, duration * .25, {alpha: 0}, false)
			.autoVisible(false)
			.ease(Quad.easeInOut)
			.delay(duration * .5);
		Actuate.tween(target, duration * .25, {alpha: 1}, false)
	...
	public static function backIn<T>(target:T, duration:Float = 1.0, translation:Int = 100):T {
		var current = Reflect.getProperty(target, "y");
		Actuate.apply(target, {
			alpha: 0,
			scaleX: .7,
			scaleY: .7,
			y: current + translation
		}).autoVisible(false);
